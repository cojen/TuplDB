...
final class _Node extends Latch implements _DatabaseAccess {
    ...
    // Links within usage list, guarded by _NodeUsageList.
    _Node mMoreUsed; // points to more recently used node
    _Node mLessUsed; // points to less recently used node

    // Links within dirty list, guarded by _NodeDirtyList.
    _Node mNextDirty;
    _Node mPrevDirty;
    ...
    _Node cloneNode() {
        _Node newNode = new _Node(mUsageList, mPage);
        newNode.mId = mId;
    ...
    void finishSplitRoot() throws IOException {
        ...
        _LocalDatabase db = mUsageList.mDatabase;
    ...
    boolean evict(_LocalDatabase db) throws IOException {
        evictCheck: {
            ...
            if (last != null) {
                ...
                if (last instanceof _GhostFrame && last.mPrevCousin == null) {
                    // Allow eviction. A full search will be required when the ghost is
                    // eventually deleted.
                    ((_GhostFrame) last).evicted();
                    break evictCheck;
                }
                releaseExclusive();
                return false;
    ...
    void splitLeafRight(_Tree tree, byte[] splitKey) throws IOException {
        ...
        _Node newNode = tree.mDatabase.allocDirtyNode(_NodeUsageList.MODE_UNEVICTABLE);
        ...
        throws IOException
    {
        ...
        _Node newNode;
        try {
            newNode = db.allocDirtyNode(_NodeUsageList.MODE_UNEVICTABLE);
