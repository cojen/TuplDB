...
class _TreeMerger {
    ...
    _TreeMerger(_LocalDatabase db, int threadCount, _Tree... sources) {
    ...
    _Tree merge() throws IOException {
    ...
    _Tree merge(ExecutorService executor) throws IOException {
        if (executor == null) {
            throw new IllegalArgumentException();
        }
        ...
        _Tree[] targets = new _Tree[mThreadCount];
    ...
    private class Worker implements Callable<Object>, Closeable {
        private final _TreeCursor mTarget;
        private final PriorityQueue<Selector> mQueue;

        Worker(_Tree target) throws IOException {
