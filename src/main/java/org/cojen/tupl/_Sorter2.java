 ...
 * 
 *
 * @author Generated by PageAccessTransformer from Sorter2.java
 */
/*P*/
class _Sorter2 {
    ...
    private final _Node[] mSortNodes;
    private int mSortNodePos;

    private final List<List<_Tree>> mSortTreeLevels;

    private ExecutorService mExecutor;
    private int mActiveThreads;

    _Sorter2(_LocalDatabase db) {
        mDatabase = db;
        mSortNodes = new _Node[SORT_NODES];
    ...
    public synchronized void add(byte[] key, byte[] value) throws IOException {
        _Node node = mSortNodes[mSortNodePos];
    ...
    public synchronized Index finish() throws IOException {
        try {
            ...
            int allTreeCount = 0;
            for (List<_Tree> trees : mSortTreeLevels) {
                allTreeCount += trees.size();
            }

            List<_Tree> allTrees = new ArrayList<>(allTreeCount);
            for (List<_Tree> trees : mSortTreeLevels) {
    ...
    private void mergeNodes() throws IOException {
        ...
        _Node[] nodes = Arrays.copyOfRange(mSortNodes, 0, len);

        executor(1).execute(() -> {
            _Tree dest;
            try {
                PriorityQueue<_Node> pq = new PriorityQueue<>
                    (nodes.length, _Sorter2::compareSortNode);
    ...
    private void mergeTrees(List<_Tree> trees, int targetLevel) throws IOException {
        ...
        executor.execute(() -> {
            _Tree merged;
            try {
                merged = _TreeMerger.merge(executor, mDatabase, MERGE_THREAD_COUNT, toMerge);
            } catch (Throwable e) {
                synchronized (this) {
