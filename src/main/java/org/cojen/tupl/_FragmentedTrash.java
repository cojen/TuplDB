...
final class _FragmentedTrash {
        ...
        throws IOException
    {
        ...
        _BTreeCursor cursor = prepareEntry(txn.txnId());
        byte[] key = cursor.key();
        try {
     ...
     */
    private _BTreeCursor prepareEntry(long txnId) throws IOException {
        ...
        _BTreeCursor cursor = new _BTreeCursor(mTrash, Transaction.BOGUS);
        try {
     ...
     */
    void remove(long txnId, _BTree index, byte[] undoEntry) throws IOException {
     ...
     * @param index index to store entry into; pass null to fully delete it instead
     */
    void remove(_BTree index, byte[] indexKey, byte[] trashKey) throws IOException {
        _BTreeCursor trashCursor = new _BTreeCursor(mTrash, Transaction.BOGUS);
        try {
