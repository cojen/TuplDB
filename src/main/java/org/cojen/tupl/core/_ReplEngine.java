...
class _ReplEngine implements RedoVisitor, ThreadFactory {
    ...
    public boolean cursorRegister(long cursorId, long indexId) throws IOException {
        ...
        Index ix = getIndex(indexId);
        if (ix != null) {
            _BTreeCursor tc = (_BTreeCursor) ix.newCursor(Transaction.BOGUS);
            tc.mKeyOnly = true;
    ...
    private static final class LockPushTask extends Worker.Task {
        private final _LocalTransaction mTxn;
        private final _Lock mLock;

        LockPushTask(_LocalTransaction txn, _Lock lock) {
            mTxn = txn;
            mLock = lock;
        }
