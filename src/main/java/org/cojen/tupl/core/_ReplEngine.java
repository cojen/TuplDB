 ...
 *
 * @author Generated by PageAccessTransformer from ReplEngine.java
 * @see _ReplController
 */
/*P*/
class _ReplEngine implements RedoVisitor, ThreadFactory {
    ...
    static {
        try {
            cDecodeExceptionHandle = MethodHandles.lookup().findVarHandle
                (_ReplEngine.class, "mDecodeException", Throwable.class);
        } catch (Throwable e) {
            throw rethrow(e);
        }
    ...
    private static MethodType storeListenerType() {
        return MethodType.methodType(void.class, _ReplEngine.class,
    ...
    final _PendingTxnFinisher mFinisher;
        ...
        throws IOException
    {
        ...
        mFinisher = new _PendingTxnFinisher(maxThreads);
        ...
        if (cStoreListenerActiveCount == 0) {
            ...
            try {
                mh = MethodHandles.lookup().findStatic
                    (_ReplEngine.class, "doRedoListenerStore", storeListenerType());
            } catch (Throwable e) {
                throw rethrow(e);
            }
    ...
    private static void redoListenerStore(_ReplEngine repl,
    ...
    private static void doRedoListenerStore(_ReplEngine repl,
                                            Transaction txn, Index ix, byte[] key, byte[] value)
    {
    ...
    private LHashTable.Obj<_LocalTransaction> doReset(boolean interrupt) throws IOException {
        ...
        synchronized (mCursors) {
            mCursors.traverse(entry -> {
                _BTreeCursor cursor = entry.mCursor;
    ...
    public boolean store(long indexId, byte[] key, byte[] value) throws IOException {
        ...
        var locker = new _Locker(mDatabase.mLockManager) {
            // Superclass doesn't support attachments by default.
            @Override
            public Object attachment() {
                return _ReplEngine.this.attachment();
            }
        };
    ...
    public boolean cursorRegister(long cursorId, long indexId) throws IOException {
        ...
        Index ix = getIndex(indexId);
        if (ix != null) {
            var tc = (_BTreeCursor) ix.newCursor(Transaction.BOGUS);
            tc.mKeyOnly = true;
    ...
    private void doCursorStore(CursorEntry ce, _LocalTransaction txn, byte[] key, byte[] value)
        throws IOException
    {
        _BTreeCursor tc = findAndRegister(ce, txn, key);
        ...
        redoListenerStore(_ReplEngine.this, txn, tc.mTree, key, value);
    }
    ...
    private void runLockPushTask(TxnEntry te, _Lock lock) {
        if (lock != null) {
            _LocalTransaction txn = te.mTxn;
            Worker w = te.mWorker;
            if (w == null) {
    ...
    private void pushPredicateLocks(_LocalTransaction txn, Object locks) {
        if (locks != null) {
            if (locks instanceof _Lock lock) {
                txn.push(lock);
            } else {
    ...
    private void pushPredicateLockArray(_LocalTransaction txn, Object locks) {
    ...
    private _BTreeCursor findAndRegister(CursorEntry ce, _LocalTransaction txn, byte[] key)
        throws IOException
    {
        _BTreeCursor tc = ce.mCursor;
     ...
     * @param e cause, which is rethrown if not due to index closure
     */
    private _BTreeCursor reopenCursor(Throwable e, CursorEntry ce) throws IOException {
        checkClosedIndex(e);
        ...
        if (ix == null) {
            ...
            tc = (_BTreeCursor) ix.newCursor(txn);
            tc.mKeyOnly = true;
            tc.mTxn = txn;
            tc.mCursorId = cursorId;
    ...
    private void reset(_BTreeCursor cursor) {
    ...
    static final class CursorEntry extends LHashTable.Entry<CursorEntry> {
        _BTreeCursor mCursor;
        Worker mWorker;
        byte[] mKey;

        void recovered(_BTreeCursor c) {
            mCursor = c;
            mKey = c.key();
        }
    }
