 ...
 *
 * @author Generated by PageAccessTransformer from BTreeCursor.java
 */
public class _BTreeCursor extends CoreValueAccessor implements Cursor {
    ...
    _BTreeCursor(_BTree tree, Transaction txn) {
        mTxn = tree.check(txn);
        mTree = tree;
    }

    _BTreeCursor(_BTree tree) {
        mTree = tree;
    }
        ...
        start: while (true) {
            ...
            while (true) {
                ...
                obtainCount: {
                    ...
                    if (child.mCachedState == _Node.CACHED_CLEAN && node.tryUpgrade()) {
                        try {
    ...
    /**
     * Called by _BTree.count. Caller must always reset the cursor after calling this method.
     */
    long countTo(_BTreeCursor high) throws IOException {
     ...
     */
    private static long countNonGhostKeys(_Node node, int lowPos, _BTreeCursor high) {
    ...
    public final boolean exists() throws IOException {
        _CursorFrame leaf = frameSharedNotSplit();
    ...
    private boolean allowRedo() {
        return !(mTree instanceof _BTree.Temp);
    }
    ...
    private boolean requireTransaction() {
        return mTree instanceof _BTree.Repl;
    }
        ...
        throws IOException
    {
        ...
        final _CursorFrame leaf = mFrame;
        _Node node = leaf.mNode;
     ...
     *
     * @return false if _BTree is closed
     */
     ...
     * @return held commitLock
     */
    private CommitLock.Shared prepareStoreUpgrade(_CursorFrame leaf, byte[] value)
        throws IOException
    {
     ...
     */
    _Node postInsert(_CursorFrame leaf, _Node node, byte[] key) throws IOException {
        int pos = leaf.mNodePos;
        int newPos = ~pos;
    ...
    final void storeFragmented(byte[] value) throws IOException {
        ...
        try {
            ...
            if (pos >= 0) {
                try {
                    node.updateLeafValue(mTree, pos, _Node.ENTRY_FRAGMENTED, value);
                } catch (Throwable e) {
                    node.releaseExclusive();
                    throw e;
                }
    ...
    public final long valueLength() throws IOException {
        ...
        long result = _BTreeValue.action(null, this, frame, _BTreeValue.OP_LENGTH, 0, null, 0, 0);
    ...
    public final void valueLength(long length) throws IOException {
        try {
            if (length <= 0) {
                ...
                doValueModify(_BTreeValue.OP_SET_LENGTH, length, EMPTY_BYTES, 0, 0);
            }
        } catch (IllegalStateException e) {
            valueCheckOpen();
            throw e;
    ...
    protected final int doValueRead(long pos, byte[] buf, int off, int len) throws IOException {
        ...
        long result = _BTreeValue.action(null, this, frame, _BTreeValue.OP_READ, pos, buf, off, len);
        frame.mNode.releaseShared();
        return (int) result;
    }
    ...
    protected final void doValueWrite(long pos, byte[] buf, int off, int len) throws IOException {
        try {
            doValueModify(_BTreeValue.OP_WRITE, pos, buf, off, len);
        } catch (IllegalStateException e) {
            valueCheckOpen();
            throw e;
        }
    ...
    protected final void doValueClear(long pos, long length) throws IOException {
        try {
            doValueModify(_BTreeValue.OP_CLEAR, pos, EMPTY_BYTES, 0, length);
        } catch (IllegalStateException e) {
            valueCheckOpen();
            throw e;
        }
        ...
        throws IOException
    {
        ...
        try {
            _BTreeValue.action(undoTxn, this, leaf, op, pos, buf, off, len);
            _Node node = leaf.mNode;

            if (op == _BTreeValue.OP_SET_LENGTH && node.shouldLeafMerge()) {
    ...
    @Override
    public final _BTreeCursor copy() {
        _BTreeCursor copy = copyNoValue();
        copy.mKeyOnly = mKeyOnly;
        copy.mValue = ViewUtils.copyValue(mValue);
        return copy;
    }

    private _BTreeCursor copyNoValue() {
        var copy = new _BTreeCursor(mTree, mTxn);
        _CursorFrame frame = mFrame;
        if (frame != null) {
            var frameCopy = new _CursorFrame();
     ...
     */
    private void reachedEnd(_Node node) throws ClosedIndexException {
     ...
     */
    final void appendTransfer(_BTreeCursor source) throws IOException {
        ...
        try {
            ...
            doRedo: {
                ...
                if (txn == null) {
                    ...
                    redo = txn.mRedo;
                    if (redo == null) {
                        // _RedoWriter can be null if the transaction is used for redo recovery
    ...
    final boolean compact(long highestNodeId, CompactionObserver observer) throws IOException {
        ...
        var frameNodes = new _Node[height];
     ...
     */
    private _Node compactFragmentedEntry(final _CursorFrame frame, _Node node, boolean isKey,
                                        long highestNodeId)
        throws IOException
    {
        ...
        while (true) {
            int result;
            try {
                result = _BTreeValue.compactCheck(frame, isKey, pos, highestNodeId);
            } catch (Throwable e) {
                node.releaseShared();
                throw e;
            }
            ...
            if (result > -1) {
                ...
                try {
                    _BTreeValue.touch(frame, isKey, pos);
                } finally {
                    shared.release();
                }
     ...
     * one.
     */
    public final boolean equalPositions(_BTreeCursor other) {
        if (this == other) {
            return true;
        }
    ...
    final boolean verify(final int height, VerificationObserver observer) throws IOException {
        if (height > 0) {
            final var stack = new _Node[height];
            while (key() != null) {
     ...
     */
    final boolean notSplitDirtyUpgrade(final _CursorFrame frame, boolean exclusive)
        throws IOException
    {
