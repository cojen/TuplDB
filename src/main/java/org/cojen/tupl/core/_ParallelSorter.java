...
final class _ParallelSorter implements Sorter, _Node.Supplier {
    ...
    private static final class Level {
        ...
        Level(int levelNum) {
            mLevelNum = levelNum;
            mTrees = new _BTree[LEVEL_MIN_SIZE];
        }
    ...
    public synchronized void add(byte[] key, byte[] value) throws IOException {
        ...
        try {
            _Node node;
            if (mSortTreesSize == 0) {
                _BTree sortTree = allocSortTree();
                (mSortTrees = new _BTree[MIN_SORT_TREES])[0] = sortTree;
                mSortTreesSize = 1;
                node = sortTree.mRoot;
            } else {
                _BTree sortTree = mSortTrees[mSortTreesSize - 1];
    ...
    public Scanner<Entry> finishScan(Scanner<Entry> src) throws IOException {
        return finishScan(new _SortScanner(mDatabase), src);
    }
    ...
    public Scanner<Entry> finishScanReverse(Scanner<Entry> src) throws IOException {
        return finishScan(new _SortReverseScanner(mDatabase), src);
    }

    private Scanner<Entry> finishScan(_SortScanner dst) throws IOException {
        try {
            _BTree tree = doFinish(dst);
            if (tree != null) {
                finishComplete(true, true);
                dst.ready(tree);
            }
    ...
    private Scanner<Entry> finishScan(_SortScanner dst, Scanner<Entry> src) throws IOException {
        if (src == null) {
    ...
    private _BTree doFinish(_SortScanner dst) throws IOException {
        ...
        synchronized (this) {
            ...
            final _BTree[] sortTrees = mSortTrees;
            ...
            levels = null;
            finishLevel.mSize = 0;
            _BTreeMerger merger = newTreeMerger(allTrees, finishLevel, finishLevel);
        ...
        throws IOException
    {
        ...
        if (mSortTreePoolSize > 0) {
            do {
                _BTree tree = mSortTreePool[--mSortTreePoolSize];
    ...
    private void runDropTasks(List<_BTree> toDrop) throws IOException {
    ...
    private void mergeSortTrees() throws IOException {
        ...
        final _BTree dest = mDatabase.newTemporaryIndex();

        final _BTree[] sortTrees = mSortTrees;
        final int size = mSortTreesSize;
        mSortTrees = new _BTree[MAX_SORT_TREES];
        mSortTreesSize = 0;
     ...
     */
    private void doMergeSortTrees(Merger merger, _BTree[] sortTrees, int size, _BTree dest)
        throws IOException
    {
        Throwable ex = null;

        final _BTreeCursor appender = dest.newCursor(Transaction.BOGUS);
        try {
            appender.firstLeaf();
    ...
    private static void siftDown(_BTree[] sortTrees, int size, int pos, _BTree element)
        throws IOException
    {
        int half = size >>> 1;
        while (pos < half) {
    ...
    private _BTreeMerger newTreeMerger(_BTree[] trees, Level level, Level nextLevel) {
        return new _BTreeMerger(mDatabase, trees, mExecutor, MERGE_THREAD_COUNT) {
            @Override
