 ...
 *
 * @author Generated by PageAccessTransformer from BTree.java
 */
class _BTree extends Tree implements View, Index {
    ...
    _BTree(_LocalDatabase db, long id, byte[] idBytes, _Node root) {
        mDatabase = db;
    ...
    /**
     * Unnamed tree which prohibits redo durability. The _BTreeCursor class checks this with the
     * allowRedo method.
     */
    static final class Temp extends _BTree {
    ...
    /**
     * _BTree which requires an explicit transaction when none is specified, excluding
     * loads. The _BTreeCursor class checks this with the requireTransaction method.
     */
    static final class Repl extends _BTree {
    ...
    @Override
    public _BTreeCursor newCursor(Transaction txn) {
        return new _BTreeCursor(this, txn);
    }
    ...
    public long count(byte[] lowKey, boolean lowInclusive,
        ...
        throws IOException
    {
        _BTreeCursor cursor = newCursor(Transaction.BOGUS);
        _BTreeCursor high = null;
        try {
    ...
    public final void store(Transaction txn, byte[] key, byte[] value) throws IOException {
        keyCheck(key);
        _BTreeCursor cursor = newCursor(txn);
        try {
            cursor.mKeyOnly = true;
    ...
    public final boolean insert(Transaction txn, byte[] key, byte[] value) throws IOException {
        ...
        _BTreeCursor cursor = newCursor(txn);
        try {
            return cursor.findAndModify(key, _BTreeCursor.MODIFY_INSERT, value);
        } finally {
            cursor.reset();
        }
    }
    ...
    public final boolean replace(Transaction txn, byte[] key, byte[] value) throws IOException {
        ...
        _BTreeCursor cursor = newCursor(txn);
        try {
            return cursor.findAndModify(key, _BTreeCursor.MODIFY_REPLACE, value);
        } finally {
            cursor.reset();
        }
    }
    ...
    public final boolean update(Transaction txn, byte[] key, byte[] value) throws IOException {
        ...
        try {
            ...
            return cursor.findAndModify(key, _BTreeCursor.MODIFY_UPDATE, value);
        } finally {
            cursor.reset();
        }
    }
     ...
     */
    final _Node close(boolean forDelete, boolean rootLatched) {
     ...
     * @return root node if forDelete; null if already closed
     */
    private _Node close(boolean forDelete, boolean rootLatched, boolean force) {
     ...
     */
    static _BTree graftTempTree(_BTree lowTree, _BTree highTree) throws IOException {
        ...
        _BTreeCursor lowCursor, highCursor;
    ...
    private static _BTree doGraftTempTree(_BTree lowTree, _BTree highTree,
                                         _BTreeCursor lowCursor, _BTreeCursor highCursor)
        throws IOException
    {
        ...
        _Node survivorNode;
        try {
            var split = new _Split(lowTree == survivor, victimNode);
        ...
        if (rootNode.numKeys() == 1 && rootNode.isInternal()) {
            ...
            tryMerge: {
                if (leftNode.isLeaf()) {
                    // See _BTreeCursor.mergeLeaf method.
    ...
    final void rootSwap(Tree other) throws IOException {
        rootSwap((_BTree) other);
    }

    private void rootSwap(_BTree other) throws IOException {
        CommitLock.Shared shared = mDatabase.commitLock().acquireShared();
        try {
            final _Node aRoot = mRoot;
            final _Node bRoot = other.mRoot;
            ...
            final _Node aTail = mStubTail;
            final _Node bTail = other.mStubTail;
    ...
    final _LocalTransaction check(Transaction txn) throws IllegalArgumentException {
        if (txn instanceof _LocalTransaction local) {
