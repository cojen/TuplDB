...
public final class _LockManager {
    // Parameter passed to Bucket.tryLock. For new _Lock instances, value will be stored as-is
    ...
    final _DetachedLockImpl newDetachedLock(_LocalTransaction owner) {
        var lock = new _DetachedLockImpl();
    ...
    static final class Bucket extends Latch {
        ...
        _Lock lockFor(long indexId, byte[] key, int hash) {
            _Lock[] entries = mEntries;
            int index = hash & (entries.length - 1);
            for (_Lock e = entries[index]; e != null; e = e.mLockNext) {
                if (e.matches(indexId, key, hash)) {
                    return e;
                }
            }
        ...
        _Lock lockAccess(long indexId, byte[] key, int hash) {
            _Lock[] entries = mEntries;
            int index = hash & (entries.length - 1);
            for (_Lock lock = entries[index]; lock != null; lock = lock.mLockNext) {
            ...
            try {
                ...
                for (_Lock e = entries[index]; e != null; e = e.mLockNext) {
                    if (e.matches(lock.mIndexId, lock.mKey, hash)) {
                        ...
                        if (ghost instanceof _GhostFrame gf) {
                            e.setGhostFrame(gf);
                        }
                        return;
                    }
        ...
        /**
         * Caller must hold latch and ensure that _Lock is in this Bucket.
         *
        ...
        private _Lock[] rehash(_Lock[] entries) {
            ...
            var newEntries = new _Lock[capacity];
