 ...
 * 
 *
 * @author Generated by PageAccessTransformer from RowPredicateLockImpl.java
 */
/*P*/
final class _RowPredicateLockImpl<R> implements RowPredicateLock<R> {
    ...
    static {
        try {
            ...
            cNewestVersionHandle = lookup.findVarHandle
                (_RowPredicateLockImpl.class, "mNewestVersion", StripedVersionLock.class);

            cLockNextHandle = lookup.findVarHandle(_Lock.class, "mLockNext", _Lock.class);

            cLastEvaluatorHandle = lookup.findVarHandle
                (_RowPredicateLockImpl.class, "mLastEvaluator", Evaluator.class);
    ...
    {
        // Assume this method is called by _ReplEngine, in which case txn is never UNSAFE.
        ...
        try {
            for (Evaluator<R> e = mLastEvaluator; ; e = e.mPrev) {
                ...
                if (lock != null) {
                    if (locks == null) {
                        locks = lock;
                    } else if (locks instanceof _Lock first) {
                        var array = new _Lock[4];
                        array[0] = first;
                        array[1] = lock;
                        locks = array;
                    } else {
                        var array = (_Lock[]) locks;
                        if (numLocks >= array.length) {
                            var newArray = new _Lock[array.length << 1];
    ...
    private void unlockUnowned(_Lock lock) {
        _LockManager.Bucket bucket = mManager.getBucket(lock.mHashCode);
    ...
    private void addEvaluator(final Transaction txn, final Evaluator<R> evaluator)
        throws LockFailureException
    {
        final var local = (_LocalTransaction) txn;
        evaluator.mLock = this;
        mManager.initDetachedLock(evaluator, local);

        // Adopts a similar concurrent linked list design as used by _CursorFrame.bind.
        add: {
    ...
    private void remove(final Evaluator<R> lock) {
        // Adopts a similar concurrent linked list design as used by _CursorFrame.unbind.
    ...
    private static final class StripedVersionLock extends VersionLock {
         ...
         */
        boolean await(long indexId, Evaluator<?> evaluator, _LocalTransaction txn)
            throws LockFailureException
        {
            boolean result = true;
            var stripes = (VersionLock[]) cStripesHandle.getAcquire(this);
    ...
    private static class VersionLock extends _DetachedLockImpl implements Closer {
         ...
         * @return true if should discard
         */
        final boolean doAwait(long indexId, Evaluator<?> evaluator, _LocalTransaction txn)
            throws LockFailureException
        {
            final _LockManager.Bucket bucket = mBucket;
            ...
            waitCheck: {
                if (lockers != null) {
                    if (lockers instanceof _Locker locker) {
                        if (locker != txn) {
                    ...
                    } else for (_Locker locker : (_Locker[]) lockers) {
                        if (locker != txn) {
                            conflict = findAnyConflict(locker, indexId, evaluator);
                            if (conflict != null) {
                                // Must wait.
            ...
            if (deadlock != null) {
                ...
                Object attachment = conflict.findOwnerAttachment
                    (txn, false, _LockManager.TYPE_EXCLUSIVE);
        ...
        private static LockFailureException failed(_LocalTransaction txn, int w)
            throws DeadlockException
        {
            LockResult result = w < 0 ? LockResult.INTERRUPTED : LockResult.TIMED_OUT_LOCK;
            return txn.failed(_LockManager.TYPE_EXCLUSIVE, result, txn.mLockTimeoutNanos);
        }
    }
    ...
    public static abstract class Evaluator<R>
        extends _DetachedLockImpl implements RowPredicate<R>, Closer
    {
