...
final class _Lock {
     ...
     */
    boolean isAvailable(_Locker locker) {
     ...
     */
    LockResult check(_Locker locker) {
     ...
     */
    void unlock(_Locker locker, _LockManager.LockHT ht) {
        if (mOwner == locker) {
            int count = mLockCount;
     ...
     */
    void doUnlock(_Locker locker, _LockManager.LockHT ht) {
        if (mOwner == locker) {
            doUnlockOwned(ht);
        } else {
            doUnlockShared(locker, ht);
     ...
     * @throws IllegalStateException if lock not held
     */
    private void doUnlockShared(_Locker locker, _LockManager.LockHT ht) {
        int count = mLockCount;
     ...
     * shared locks
     */
    void unlockToShared(_Locker locker, Latch latch) {
        if (mOwner == locker) {
            int count = mLockCount;
     ...
     * @throws IllegalStateException if lock not held
     */
    void doUnlockToUpgradable(_Locker locker, Latch latch) {
        if (mOwner != locker) {
            if (isClosed(locker)) {
                latch.releaseExclusive();
                return;
    ...
    void setSharedLocker(_Locker owner) {
    ...
    {
        var detector = new _DeadlockDetector(locker);
        if (detector.scan()) {
    ...
    Object findOwnerAttachment(_Locker locker, int lockType) {
        ...
        if (sharedObj instanceof _Locker) {
            return ((_Locker) sharedObj).attachment();
        }
     ...
     */
    private boolean isSharedLocker(_Locker locker) {
        Object sharedObj = mSharedLockersObj;
        if (sharedObj == locker) {
            return true;
        }
    ...
    private void addSharedLocker(int count, _Locker locker) {
    ...
    private static _Locker lockerHTgetOne(LockerHTEntry[] entries) {
