...
final class _TransactionContext extends Latch implements Flushable {
     ...
     */
    void redoLock(_RedoWriter redo, byte op, long txnId, long indexId, byte[] key)
        throws IOException
    {
        keyCheck(key);
        redo.opWriteCheck(null);
     ...
     * @return non-zero position if caller should call txnCommitSync; -1 if transaction is pending
     */
    long redoStoreCommitFinal(_LocalTransaction txn, long indexId, byte[] key, byte[] value)
        throws IOException
    {
        keyCheck(key);
     ...
     * @return non-zero position if caller should call txnCommitSync; -1 if transaction is pending
     */
    long redoDeleteCommitFinal(_LocalTransaction txn, long indexId, byte[] key)
        throws IOException
    {
        keyCheck(key);
     ...
     */
    void redoPrepare(_RedoWriter redo, long txnId, long prepareTxnId,
    ...
    void redoCustomLock(_RedoWriter redo, long txnId, int handlerId, byte[] message,
