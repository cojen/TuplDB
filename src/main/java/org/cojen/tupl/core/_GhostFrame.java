 ...
 * to anything. Register the custom action with the _LockManager.ghosted method.
 *
 * @author Generated by PageAccessTransformer from GhostFrame.java
 */
/*P*/
class _GhostFrame extends _CursorFrame {
    /**
     ...
     */
    void action(_LocalDatabase db, Latch latch, _Lock lock) {
        byte[] key = lock.mKey;
        ...
        doDelete: try {
            ...
            if (node == null) {
                ...
                // _Node cannot be dirtied, or quick delete shouldn't be performed. In either
                // case, a full cursor is required, so delete the slow way.
                _CursorFrame.popAll(this);
                node.releaseExclusive();
            } else {
            ...
            while (true) {
                ...
                if (!(ix instanceof _BTree tree)) {
                    // Assume index was deleted.
                    break;
                }
                var c = new _BTreeCursor(tree);
