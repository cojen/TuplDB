...
class ReplEngine implements RedoVisitor, ThreadFactory {
     ...
     */
    private boolean awaitPreparedTransactions(List<UndoLog> undoLogs, long nanosTimeout) {
        final LockManager lockManager = mDatabase.mLockManager;

        // Temporary locker used for signal await.
        final Locker locker = new Locker(lockManager);

        boolean finished = true;

        Iterator<UndoLog> it = undoLogs.iterator();
        while (it.hasNext()) {
            UndoLog undoLog = it.next();
