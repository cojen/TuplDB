...
/**
 * Core _BTreeCursor ValueAccessor implementation.
 *
 * @author Generated by PageAccessTransformer from BTreeValue.java
 */
final class _BTreeValue {
     ...
     */
    static int compactCheck(final _CursorFrame frame, boolean isKey, long pos, long highestNodeId)
        throws IOException
    {
     ...
     * @param isKey true to examine the key instead of the value
     */
    static void touch(_CursorFrame frame, boolean isKey, final long pos) throws IOException {
    ...
    static long action(_LocalTransaction txn, _BTreeCursor cursor, _CursorFrame frame,
        ...
        throws IOException
    {
        while (true) {
            ...
            // by the _LocalDatabase.fragment method.
                ...
                if (endPos <= fLen) {
                    ...
                    if ((fHeader & 0x01) != 0) try {
                        ...
                        if (db.levelCap(levels) < newLen) {
                            ...
                            var newNodes = new _Node[newLevels - levels];
     ...
     */
    private static void readMultilevelFragments(long pos, int level, _Node inode,
     ...
     */
    private static void writeMultilevelFragments(_LocalTransaction txn, _BTreeCursor cursor,
                                                 long pos, long ppos, int level, _Node inode,
                                                 final byte[] b, int bOff, int bLen)
        throws IOException
    {
        _LocalDatabase db = inode.getDatabase();
     ...
     */
    private static void clearMultilevelFragments(_LocalTransaction txn, _BTreeCursor cursor,
                                                 long pos, long ppos, int level, final _Node inode,
     ...
     * @param inode exclusively latched parent inode; always released by this method
     */
    private static void touchMultilevelFragment(long ppos, int level, _Node inode)
        throws IOException
    {
        _LocalDatabase db = inode.getDatabase();
     ...
     */
    private static void tryIncreaseLengthField(final _BTreeCursor cursor, final _CursorFrame frame,
        ...
        throws IOException
    {
        ...
        final _BTree tree = cursor.mTree;
        ...
        try {
            ...
            p_bytePut(page, vHeaderLoc, p_byteGet(page, vHeaderLoc) & ~_Node.ENTRY_FRAGMENTED);

            // The fragmented bit is set again by this call.
            node.updateLeafValue(tree, frame.mNodePos, _Node.ENTRY_FRAGMENTED, newValue);
        } catch (Throwable e) {
            throw releaseExclusive(node, e);
        }
     ...
     */
    private static int tryExtendDirect(final _BTreeCursor cursor, final _CursorFrame frame,
                                       final int kHeaderLoc,
                                       final int vHeaderLoc, final int vLen,
                                       final int fHeaderLoc, final long growth)
        throws IOException
     ...
     * @param fHeaderLoc location of fragmented value header
     */
    private static void compactDirectFormat(final _BTreeCursor cursor, final _CursorFrame frame,
                                            final int kHeaderLoc,
                                            final int vHeaderLoc, final int vLen,
     ...
     */
    private static _Node shiftDirectRight(_LocalDatabase db, final long page,
        ...
        throws IOException
    {
        ...
        final var fNodes = new _Node[(endLoc - startLoc) / 6];
        final int pageSize = pageSize(db, page);
