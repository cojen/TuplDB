...
import static org.cojen.tupl.core._PageManager.ALLOC_NORMAL;
import static org.cojen.tupl.core._PageManager.ALLOC_RESERVE;
import static org.cojen.tupl.core.DirectPageOps.*;
import static org.cojen.tupl.core.Utils.scramble;

/**
 * Used by {@link _PageManager} to implement free lists.
 *
 * @author Generated by PageAccessTransformer from PageQueue.java
 */
final class _PageQueue implements IntegerRef {
    /*
    ...
    */
    ...
    /**
     * Returns true if a valid _PageQueue instance is encoded in the header.
     */
    ...
    static _PageQueue newRegularFreeList(_PageManager manager) {
        return new _PageQueue(manager, ALLOC_NORMAL, false, null);
    }
    ...
    static _PageQueue newRecycleFreeList(_PageManager manager) {
        return new _PageQueue(manager, ALLOC_NORMAL, true, null);
    }
    ...
    /**
     * Delete all available pages, effectively deleting this _PageQueue. Only works for page
    ...
    long tryRemove(ReentrantLock lock) throws IOException {
        ...
        // commit. _Lock acquisition order wouldn't match. Note that node is
    ...
    private long readRemoveNode(long id) throws IOException {
        ...
        if (cache == null || (node = cache.nodeMapGetAndRemove(id)) == null) {
            ...
            if (cache.mFullyMapped) {
                p_copy(node.mPage, 0, head, 0, mPageSize);
            } else {
                // Swap the pages.
                mRemoveHead = node.mPage;
                node.mPage = head;
    ...
    void append(long id, boolean force) throws IOException {
        ...
        final _IdHeap appendHeap = mAppendHeap;
        ...
        try {
            ...
            // which is called by drainAppendHeap itself. The _IdHeap has
    ...
    private void drainAppendHeap(_IdHeap appendHeap) throws IOException {
        ...
        try {
            ...
            if (cache == null || mAllocMode == ALLOC_RESERVE) {
                ...
                // _LocalDatabase.mCommitState is always performed with the commit lock held AND
    ...
    boolean verifyPageRange(long startId, long endId) throws IOException {
        final _LocalDatabase cache = mManager.mPageCache;
