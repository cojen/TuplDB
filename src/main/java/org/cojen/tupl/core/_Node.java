...
final class _Node extends Clutch implements _DatabaseAccess {
    // Note: Changing these values affects how the _LocalDatabase class handles the commit
    ...
    /*
      ...
      value, defined by _LocalDatabase.fragment.
    ...
    volatile _CursorFrame mLastCursorFrame;

    // Set by a partially completed split.
    _Split mSplit;

    _Node(_NodeGroup group, long page) {
        mGroup = group;
        mPage = page;
    }

    // Construct a stub node, latched exclusively.
    _Node(_NodeGroup group) {
        super(EXCLUSIVE);
    ...
    _Node cloneNode() {
        var newNode = new _Node(mGroup, mPage);
        newNode.id(id());
    ...
    void finishSplitRoot() throws IOException {
        ...
        _LocalDatabase db = mGroup.mDatabase;
    ...
    boolean evict(_LocalDatabase db) throws IOException {
        ...
        if (last != null) {
            ...
            do {
                if (!(frame instanceof _GhostFrame)) {
                    releaseExclusive();
                    return false;
                }
                frame = frame.mPrevCousin;
     ...
     * @page page p_closedTreePage or p_deletedTreePage
     */
    void invalidateCursors(long page) {
        invalidateCursors(page, createClosedNode(page));
    }

    private void invalidateCursors(long page, _Node closed) {
        int pos = isLeaf() ? -1 : 0;
     ...
     * @page page p_closedTreePage or p_deletedTreePage
     */
    private static _Node createClosedNode(long page) {
        var closed = new _Node(null, page);
        closed.id(CLOSED_ID);
     ...
     */
    private void retrieveKeyAtLoc(final long page, int loc, _Split split)
        throws IOException
    {
     ...
     */
    private static boolean retrieveActualKeyAtLoc(final long page, int loc,
                                                  final byte[][] akeyRef)
        throws IOException
    {
        boolean result = true;
     ...
     * @param pos position as provided by binarySearch; must be positive
     */
    void txnPreUpdateLeafEntry(_LocalTransaction txn, _BTree tree, int pos)
        throws IOException
    {
        final var page = mPage;
        final int entryLoc = p_ushortGetLE(page, searchVecStart() + pos);
     ...
     */
    private static long childIdByOffset(long page, int offset) {
    ...
    {
        if (vlength < 0) {
            // This method is called by _BTreeValue.action to create a new value.
     ...
     * held exclusively.
     */
    private void unrebindSplitFrames(_Split split, _Node sibling) {
        for (_CursorFrame frame = mLastCursorFrame; frame != null; ) {
            // Capture previous frame from linked list before changing the links.
            _CursorFrame prev = frame.mPrevCousin;
        ...
        for (_CursorFrame frame = sibling.mLastCursorFrame; frame != null; ) {
            // Capture previous frame from linked list before changing the links.
            _CursorFrame prev = frame.mPrevCousin;
     ...
     */
    void updateLeafValue(_BTree tree, int pos, int vfrag, byte[] value)
        throws IOException
    {
        var page = mPage;
     ...
     * @param encodedLen length of new entry to allocate
     */
    void splitLeafAscendingAndCopyEntry(_BTree tree, _Node snode, int spos, int encodedLen)
        throws IOException
    {
        ...
        _Node newNode = tree.mDatabase.allocDirtyNode(_NodeGroup.MODE_UNEVICTABLE);
        ...
        throws IOException
    {
        ...
        _Node newNode;
        try {
            newNode = db.allocDirtyNode(_NodeGroup.MODE_UNEVICTABLE);
    ...
    private _Split newSplitLeft(_Node newNode) {
        var split = new _Split(false, newNode);
    ...
    private _Split newSplitRight(_Node newNode) {
        var split = new _Split(true, newNode);
