...
class _Locker implements _DatabaseAccess { // weak access to database
    ...
    // _Locker is currently waiting to acquire this lock. Used for deadlock detection.
    _Lock mWaitingFor;
    ...
    static {
        try {
            cTailBlockHandle = MethodHandles.lookup().findVarHandle
                (_Locker.class, "mTailBlock", Object.class);
        } catch (Throwable e) {
        ...
        throws DeadlockException
    {
        ...
        if (!result.isHeld()) {
            try {
                ...
                // shouldn't be DEADLOCK when the timeout is 0. See _Lock class.
                if (nanosTimeout != 0) {
    ...
    final _Lock doLockSharedNoPush(long indexId, byte[] key) throws LockFailureException {
        ...
        _LockManager.Bucket bucket = mManager.getBucket(hash);
    ...
    private static _Lock peek(Object tailObj) {
        return (tailObj instanceof _Lock lock) ? lock : ((Block) tailObj).last();
    }
    ...
    public final void unlock() {
        ...
        if (tailObj instanceof _Lock lock) {
            ParentScope parent = mParentScope;
    ...
    final void push(_Lock lock) {
        ...
        } else if (tailObj instanceof _Lock tailLock) {
    ...
    final void pushUpgrade(_Lock lock) {
        Object tailObj = mTailBlock;
        if (tailObj instanceof _Lock tailLock) {
     ...
     * one which is allowed to modify the _Locker.
     *
        ...
        if (tailObj != null) {
            if (tailObj instanceof _Lock lock) {
                if (lock.mIndexId == indexId && predicate.test(lock.mKey) &&
                    ((int) _Lock.cLockCountHandle.getAcquire(lock)) < 0)
                {
                    return lock;
                }
            } else {
                ...
                do {
                    _Lock lock = block.findAnyConflict(indexId, predicate);
                    if (lock != null) {
                        return lock;
                    }
    ...
    static final class Block {
        ...
        Block(_Lock first, _Lock second) {
            var locks = new _Lock[FIRST_BLOCK_CAPACITY];
            locks[0] = first;
            locks[1] = second;
            mLocks = locks;
