 ...
 * the tree.
 *
 * <p>Cursor frames are bound to Nodes, managed by a concurrent linked list. The _CursorFrame
 * itself is a node in the linked list. The unbind operation can be performed without holding
 * any sort of latch on the _Node, but other operations can require a latch. Although the
 * concurrent linked list design doesn't require this, the latching is necessary to ensure that
 * the _Node itself is stable.
 *
