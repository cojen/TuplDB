...
final class _TreeValue {
        ...
        throws IOException
    {
        ...
        if ((vHeader & 0x20) == 0) {
            ...
            // Ghost. This case isn't expected, because _TreeCursor.compact doesn't
            // call this method for non-fragmented values.
            return -1;
        }

        if ((vHeader & _Node.ENTRY_FRAGMENTED) == 0) {
            // Not fragmented. This case isn't expected, because _TreeCursor.compact doesn't
            // call this method for non-fragmented values.
            return pos >= len ? -1 : 0;
        }
    ...
    static long action(_LocalTransaction txn, _TreeCursor cursor, _CursorFrame frame,
            ...
            nonFrag: {
                ...
                decodeLen: if (vHeader >= 0) {
                    ...
                    if ((vHeader & 0x20) == 0) {
                        ...
                        if (b == TOUCH_VALUE) {
                            // This case isn't expected, because _TreeCursor.compact doesn't
                            // call this method for non-fragmented values.
                            return 0;
                        }
            ...
            case OP_WRITE:
                ...
                if (endPos <= fLen) {
                    ...
                    if (b == TOUCH_VALUE) {
                        ...
                        // This case isn't expected, because _TreeCursor.compact holds the node
     ...
     */
    private static void writeMultilevelFragments(_LocalTransaction txn, _TreeCursor cursor,
     ...
     */
    private static void clearMultilevelFragments(_LocalTransaction txn, _TreeCursor cursor,
     ...
     * @param fHeaderLoc location of fragmented value header
     */
    private static void compactDirectFormat(final _TreeCursor cursor, final _CursorFrame frame,
                                            final int kHeaderLoc,
                                            final int vHeaderLoc, final int vLen,
