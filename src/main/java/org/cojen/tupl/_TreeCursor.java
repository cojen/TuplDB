 ...
 */
class _TreeCursor extends AbstractBlob implements CauseCloseable, Cursor {
    ...
    public final long valueLength() throws IOException {
        ...
        long result = _TreeValueBlob.action(this, frame, _TreeValueBlob.OP_LENGTH, 0, null, 0, 0);
    ...
    public final void setValueLength(long length) throws IOException {
        ...
        try {
            ...
            try {
                notSplitDirty(leaf);
                _TreeValueBlob.action
                    (this, leaf, _TreeValueBlob.OP_SET_LENGTH, length, EMPTY_BYTES, 0, 0);
                leaf.mNode.releaseExclusive();
            } finally {
                shared.release();
            }
    ...
    final int blobRead(long pos, byte[] buf, int off, int len) throws IOException {
        ...
        int result = (int) _TreeValueBlob.action
            (this, frame, _TreeValueBlob.OP_READ, pos, buf, off, len);
        frame.mNode.releaseShared();
        return result;
    }
    ...
    final void blobWrite(long pos, byte[] buf, int off, int len) throws IOException {
        ...
        try {
            ...
            try {
                notSplitDirty(leaf);
                _TreeValueBlob.action(this, leaf, _TreeValueBlob.OP_WRITE, pos, buf, off, len);
                leaf.mNode.releaseExclusive();
            } finally {
                shared.release();
            }
            ...
            while (true) {
                try {
                    ...
                    if (nodePos >= 0 && node.isFragmentedLeafValue(nodePos)) {
                        ...
                        long pos = 0;
                        while (true) {
                            int result = _TreeValueBlob.compactCheck(frame, pos, highestNodeId);
                            if (result < 0) {
                                break;
                            }
                            if (result > 0) {
                                node.releaseShared();
                                node = null;
                                blobWrite(pos, _TreeValueBlob.TOUCH_VALUE, 0, 0);
                                frame = leafSharedNotSplit();
                                node = frame.mNode;
