 ...
 */
class _TreeCursor extends AbstractValueAccessor implements Cursor {
    ...
    private boolean allowRedo() {
        return !(mTree instanceof _Tree.Temp);
    }
    ...
    private boolean requireTransaction() {
        return mTree instanceof _Tree.Repl;
    }
    ...
    public final void valueLength(long length) throws IOException {
        try {
            if (length <= 0) {
                ...
                doValueModify(_TreeValue.OP_SET_LENGTH, length, EMPTY_BYTES, 0, 0);
            }
        } catch (IllegalStateException e) {
            valueCheckOpen();
            throw e;
    ...
    final void doValueWrite(long pos, byte[] buf, int off, int len) throws IOException {
        try {
            doValueModify(_TreeValue.OP_WRITE, pos, buf, off, len);
        } catch (IllegalStateException e) {
            valueCheckOpen();
            throw e;
        }
    ...
    final void doValueClear(long pos, long length) throws IOException {
        try {
            doValueModify(_TreeValue.OP_CLEAR, pos, EMPTY_BYTES, 0, length);
        } catch (IllegalStateException e) {
            valueCheckOpen();
            throw e;
        }
