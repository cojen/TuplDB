...
class _TreeCursor extends AbstractValueAccessor implements CauseCloseable, Cursor {
    ...
    private _Node toNextAny(_CursorFrame frame) throws IOException {
        ...
        while (true) {
            _CursorFrame parentFrame = frame.peek();
     ...
     */
    protected final void store(final _LocalTransaction txn, final _CursorFrame leaf,
                               final byte[] value)
        throws IOException
    {
        byte[] key;
    ...
    final boolean deleteAll() throws IOException {
        ...
        while (true) {
            ...
            try {
                ...
                // Releases latch if an exception is thrown.
                _Node node = notSplitDirty(mLeaf);
    ...
    public final void setValueLength(long length) throws IOException {
        try {
            if (length <= 0) {
                ...
                doValueModify(storeMode(), mTxn, _TreeValue.OP_SET_LENGTH,
    ...
    final void doValueWrite(long pos, byte[] buf, int off, int len) throws IOException {
        try {
            doValueModify(storeMode(), mTxn, _TreeValue.OP_WRITE, pos, buf, off, len);
        } catch (IllegalStateException e) {
            valueCheckOpen();
            throw e;
        }
     ...
     */
    private void doValueModify(int mode, _LocalTransaction txn, int op,
        ...
        throws IOException
    {
        ...
        try {
            ...
            if (mode <= 1 && txn.durabilityMode() != DurabilityMode.NO_REDO) {
                ...
                _TransactionContext context = txn.mContext;
    ...
    public final boolean equalPositions(_TreeCursor other) {
        ...
        _CursorFrame thisFrame = mLeaf;
        _CursorFrame otherFrame = other.mLeaf;
        while (true) {
            if (thisFrame == null) {
     ...
     * @throws UnpositionedCursorException if unpositioned
     */
    final _CursorFrame leafSharedNotSplit() throws IOException {
        _CursorFrame leaf = leaf();
        _Node node = leaf.acquireShared();
        if (node.mSplit != null) {
     ...
     */
    private _Node latchToChild(_Node parent, int childPos) throws IOException {
     ...
     * @return child node, possibly split
     */
    private _Node latchChildRetainParent(_Node parent, int childPos) throws IOException {
