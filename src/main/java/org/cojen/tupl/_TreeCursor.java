...
class _TreeCursor extends AbstractValueAccessor implements CauseCloseable, Cursor {
     ...
     */
    private CommitLock.Shared prepareDeleteUpgrade(_CursorFrame leaf) throws IOException {
        if (leaf.mNodePos < 0) {
            // Entry doesn't exist, so nothing to do.
     ...
     * @return held commitLock
     */
    private CommitLock.Shared prepareStoreUpgrade(_CursorFrame leaf) throws IOException {
        CommitLock commitLock = mTree.mDatabase.commitLock();
        CommitLock.Shared shared = commitLock.tryAcquireShared();
